...
void WaterSimulator::load_textures() {
...
void WaterSimulator::loadWaterCube(WaterCube *waterCube) { this->waterCube = waterCube; }

void WaterSimulator::loadWaterCubeParameters(WaterCubeParameters *cp) { this->cp = cp; }

void WaterSimulator::loadCollisionObjects(vector<CollisionObject *> *objects) { this->collision_objects = objects; }

/**
 * Initializes the waterCube simulation and spawns a new thread to separate
...
void WaterSimulator::init() {
    ...
    for (auto &pm : waterCube->water_particles) {
        avg_pm_position += pm.position / waterCube->water_particles.size();
    }

    CGL::Vector3D target(avg_pm_position.x, avg_pm_position.y / 2,
                         avg_pm_position.z);
    CGL::Vector3D c_dir(0., 0., 0.);
    canonical_view_distance = max(waterCube->width, waterCube->height) * 0.9;
...
void WaterSimulator::drawPhong(GLShader &shader) {
    waterCube->buildWaterCube(shader);

    for (auto particle : waterCube->water_particles) {
        particle.mesh.draw_sphere(shader, particle.position, particle.radius);
    }
}
